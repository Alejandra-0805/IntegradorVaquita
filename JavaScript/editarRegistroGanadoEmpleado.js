...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    document.getElementById("btnEditar").addEventListener("click", async () => {
        ...
        if (fechaMuerte) {
            const fMuerte = new Date(fechaMuerte);
            const fNacimiento = new Date(fechaNacimiento);
            const hoy = new Date();
            hoy.setHours(0, 0, 0, 0);

            if (fMuerte <= fNacimiento) {
                Swal.fire({
                    icon: "warning",
                    title: "Fecha inválida",
                    text: "La fecha de muerte/baja debe ser mayor que la fecha de nacimiento.",
                    timer: 4000,
                    showConfirmButton: false
                });
                return;
            ...
            if (fMuerte > hoy) {
                Swal.fire({
                    icon: "warning",
                    title: "Fecha inválida",
                    text: "La fecha de muerte no puede ser futura.",
                    timer: 4000,
                    showConfirmButton: false
                });
                return;
