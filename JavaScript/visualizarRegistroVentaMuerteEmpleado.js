...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    ganado.forEach(item => {
      ...
      let fechaBajaDate = null;

      if (item.fechaBaja) {
        fechaBajaDate = new Date(
          item.fechaBaja[0],
          item.fechaBaja[1] - 1,
          item.fechaBaja[2]
        );
      ...
      if (fechaBajaDate && fechaBajaDate < hoy) {
        Swal.fire({
          icon: "warning",
          title: "Fecha de baja inválida",
          text: `El ganado "${g.nombre ?? g.idArete}" tiene una fecha de baja pasada.`,
          confirmButtonText: "Entendido"
        });
        console.warn("Fecha de baja pasada, no se muestra:", item);
        return;
      }
    ...
    Swal.fire({
      icon: "error",
      title: "Error...",
      text: "Error al cargar datos de la API.",
      timer: 4000,
