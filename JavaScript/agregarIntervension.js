...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    selectMedicamento.addEventListener("change", () => {
        const idSeleccionado = Number(selectMedicamento.value);

        const medEncontrado = listaMedicamentos.find(
            (m) => m.idMedicamento === idSeleccionado
        );

        descripcionInput.value = medEncontrado
            ? medEncontrado.descripcion || "Sin descripción"
            : "";
    });
