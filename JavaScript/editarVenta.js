...
document.querySelector('.btn-agregar').addEventListener('click', async () => {
  ...
  if (Number(idArete) <= 0 || Number(pesoFinal) <= 0 || Number(precioVenta) <= 0) {
    Swal.fire({
      icon: "warning",
      title: "Valores inválidos",
      text: "El número de arete, el peso final y el precio deben ser mayores a cero.",
      timer: 4000,
      showConfirmButton: false,
    });
    return;
  ...
  const pesoOriginal = Number(ganadoData.peso);

  if (Number(pesoFinal) < pesoOriginal) {
    Swal.fire({
      icon: "warning",
      title: "Peso inválido",
      text: `El peso final (${pesoFinal} kg) no puede ser menor al peso registrado originalmente (${pesoOriginal} kg).`,
      timer: 4500,
      showConfirmButton: false,
    });
    return;
